...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const SAFE_TAGS = [
    "p","h1","h2","h3","h4","h5","h6",
    "strong","em","ul","ol","li","blockquote","pre","code","hr","br",
  ...
  function enhanceImages(html: string) {
    ...
    tmp = tmp.replace(/<img\b([^>]*)>/gi, (_m, attrs: string) => {
      ...
      const caption = alt
        ? `<figcaption class="${CAPTION_CLASS}">${escapeHtml(alt)}</figcaption>`
        : "";
  ...
  const publishedHuman = new Date(publishedISO).toLocaleDateString();
  ...
  return (
    <main className="mx-auto max-w-4xl px-5 py-8 md:py-12">
      {/* back link */}
      <nav className="mb-6">
        <Link href="/blog" className="inline-flex items-center gap-2 text-sm text-emerald-700 hover:underline">
      ...
      <section
        ...
        style={{
          backgroundImage: post.social_image_url ? `url(${post.social_image_url})` : undefined,
      ...
      >
        ...
        {!post.social_image_url && <div className="absolute inset-0 bg-emerald-600/20" />}
      ...
      <article className="mt-8">
        <div
          className="
            ...
            /* Generic figure styling so even existing <figure>…<img> gets the look */
            [&_figure]:my-6
      ...
      <aside className="mt-10 flex flex-col items-start justify-between gap-4 rounded-xl border border-emerald-100 bg-emerald-50 p-4 sm:flex-row sm:items-center">
        <div>
          <p className="text-sm font-medium text-emerald-900">Share this article</p>
      ...
      {/* Tailwind safelist helper (optional): ensures figure classes are kept in builds */}
      <div className="hidden">
        {/* These classes mirror FIGURE_CLASS & CAPTION_CLASS so Tailwind JIT never drops them */}
