...
class ProductRatingListCreateView(generics.ListCreateAPIView):
    """API view for listing and creating product ratings"""
    serializer_class = ProductRatingSerializer

    def get_permissions(self):
        # Cho phép tất cả người dùng xem danh sách đánh giá,
        # nhưng chỉ người đăng nhập mới được tạo mới.
        if self.request.method == 'GET':
            return [AllowAny()]
        return [IsAuthenticated()]

    def get_queryset(self):
        # Lọc danh sách đánh giá theo sản phẩm
    ...
    def perform_create(self, serializer):
        ...
        # Ngăn người dùng đánh giá trùng một sản phẩm
...
class ProductRatingDetailView(generics.RetrieveUpdateDestroyAPIView):
    """API view for retrieving, updating and deleting a specific product rating"""
    serializer_class = ProductRatingDetailSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        # Người dùng chỉ có thể xem/sửa/xóa đánh giá của chính họ
